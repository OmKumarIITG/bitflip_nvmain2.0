...
${bold}${green}BitIndexing Run Script:${normal}
...
echo -e -n "${bold}${blue}BitIndexing...${normal}
	inCim:		$inCim
...
for var in ${FileRows[@]}; do
	...
	if [ $inCim -eq 0 ]; then
		fout=BIT-CPU\_$var\_$tag
	else
		fout=BIT-CIM\_$var\_$tag
	fi

	g++ $optimization \
		/nv-gem5/tests/test-progs/CDNCcim/app1_bit_indexing.cpp \
